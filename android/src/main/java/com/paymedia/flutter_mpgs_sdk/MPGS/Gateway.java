...
//    }
     ...
     * { Gateway#requestGooglePayData(PaymentsClient, PaymentDataRequest, Activity)} method.
     * <p>
     * requestCode The request code returning from the activity result
     * resultCode  The result code returning from the activity result
     * data        The intent data returning from the activity result
     * callback    An implementation of { GatewayGooglePayCallback}
     * True if handled, False otherwise
     * Gateway#requestGooglePayData(PaymentsClient, PaymentDataRequest, Activity)
     */
...
//    }
    ...
    void runGatewayRequest(final GatewayRequest request, final GatewayCallback callback) {
        // create handler on current thread
        final Handler handler = new Handler(new Handler.Callback() {
            @Override
            public boolean handleMessage(Message msg) {
                return handleCallbackMessage(callback, msg.obj);
            }
        });
    ...
    Single<GatewayMap> runGatewayRequest(final GatewayRequest request) {
        return Single.fromCallable(new Callable<GatewayMap>() {
            @Override
